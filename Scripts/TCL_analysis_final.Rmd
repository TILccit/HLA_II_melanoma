...
knitr::opts_chunk$set(
  ...
  message = FALSE,
  warning = FALSE,
  results = 'hide',   # Suppresses printed terminal output
  out.width = "100%"  # Auto-fit figures to available width
)
pacman::p_load(ggplot2, data.table, grid, VennDiagram, cowplot, ggrepel, edgeR, DESeq2,decoupleR, tidyr, dplyr, Rtsne, pheatmap, msigdbr, plyr, RColorBrewer, rstatix, clusterProfiler, org.Hs.eg.db, enrichplot, openxlsx, tibble, viper, GSVA, pheatmap) 

source("create_sample_lfc_heatmap.R")
source("create_violin_plot.R")
source("create_violin_plot_compare_genes_vst.R")
source("perform_cluster_analysis.R")
...
We perform unsupervised clustering of the samples based on their expression of CIITA and HLA-DR genes. The first step is to visualize the clustering output.

```{r clustering pre-batch correction}
...
Dataset information+clustering was included in the DeSeq formula to address the effects of the variables. In the end, we create a heatmap to confirm that the clustering is isolating samples with high HLA expression.The HLA-II genes have been removed from the heatmap.

```{r analyses on the batch corrected dataset}
...
## DE analysis
The differentially expressed genes can be visualized in a volcano plot, where the HLA-II related genes have been removed, to focus on the other transcriptomic differences between HLA-high and HLA-low samples
...
## GSEA
Then we perform GSEA to understand the phenotype of the HLA-II positive cluster.
```{r GSEA}
...
The GSEA analysis shows that the HLA-II positive cell lines exhibit signs of dedifferentiation, associated with upregulation of inflammatory response markers

```{r dotplot and GSEA plot, echo=FALSE}
# Extract significant pathways (adjusted p-value < 0.05)
gsea_results = gsea_results_plot
sig_pathways <- gsea_results@result[gsea_results@result$p.adjust < 0.05, ]
# Visualize the results with a dot plot
dotplot(gsea_results <- GSEA(
  geneList = gene_list,           
  TERM2GENE = msigdb_hallmark_list,          
  pvalueCutoff = 0.05,            
  minGSSize = 10,                 
  maxGSSize = 500,                
  verbose = TRUE
), showCategory = 20) + ggtitle("GSEA of Hallmark Pathways")

# Plot a specific GSEA plot for a pathway (e.g., HALLMARK_APOPTOSIS)
gseaplot(gsea_results, geneSetID = "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION", title ="Enrichment of epithelial to mesenchymal transition genes (Hallmark)" )
gseaplot(gsea_results, geneSetID = "HALLMARK_TNFA_SIGNALING_VIA_NFKB", title ="Enrichment of TNFa signalling via NFKB (Hallmark)" )

c5_go_gene_sets <- msigdbr(species = "Homo sapiens", category = "C5", subcategory = "BP")
melanocyte_diff_gene_sets <- c5_go_gene_sets[grepl("GOBP_MELANOCYTE_DIFFERENTIATION", c5_go_gene_sets$gs_name), ]
# Convert to TERM2GENE format
TERM2GENE_melanocyte_diff <- melanocyte_diff_gene_sets[, c("gs_name", "gene_symbol")]

gsea_results_mel <- GSEA(
  geneList = gene_list,           
  TERM2GENE = TERM2GENE_melanocyte_diff,        
  pvalueCutoff = 0.5,            
  minGSSize = 2,                 
  maxGSSize = 500,                
  verbose = TRUE
)
gseaplot(gsea_results_mel, geneSetID = "GOBP_MELANOCYTE_DIFFERENTIATION", title ="Enrichment of melanocyte differentiation genes (GOBP)" )
```
...
We also investigated if the Melanoma multi-stage differentiation subtypes identified in Tsoi et al., Cancer Cell 2018 could be enriched in HLA-II positive melanoma cell lines. Our analysis showed an enrichment of the neural_crest-like phenotype, with a negative enrichment for the Melanocytic phenotype, compatible with what has been observed so far.

```{r multi-stage differentiation subtypes, echo=F}
###check if any of the melanoma dediff status enriches with the const+ genes (Tsoi et al, Cancer Cell 2018)
melanoma_dediff <- read.csv("../public/Melanoma_dediff_gene_set.csv")
melanoma_dediff <- melanoma_dediff[,c(2,1)]
...
combined_results_filter_less_strict <- subset(combined_results_filter_less_strict, !gene %in% HLAII_genes)
gene_list <- combined_results_filter_less_strict$log2FoldChange
names(gene_list) <- rownames(combined_results_filter_less_strict)
gene_list <- sort(gene_list, decreasing = TRUE)

gsea_dediff <- GSEA(
  geneList = gene_list,           
  TERM2GENE = melanoma_dediff,          
  pvalueCutoff = 1.5,            
  minGSSize = 0,                 
  maxGSSize = 500,                
  verbose = TRUE,pAdjustMethod = "fdr"
)
gsea_df <- gsea_dediff@result %>% mutate(logp = -log10(p.adjust))
gsea_df$ID <- factor(gsea_df$ID, 
                          levels = c("Melanocytic", "Transitory", "Neural_crest-like", "Undifferentiated"))

# Create the bubble plot
ggplot(gsea_df, aes(x = ID, y = NES, size = logp, color = logp)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  scale_color_gradient(low = "blue", high = "red") +
  scale_size_continuous(range = c(5, 10)) +  
  labs(title = "GSEA: NES and Adjusted P-value",
       y = "Normalized Enrichment Score (NES)",
...
# de-duplicate genes (keep the most variable probe/gene)
...
# Heatmap of ssGSEA scores per sample/cell line
...
# Row-scale to emphasize variation across samples
ssgsea_z <- t(scale(t(ssgsea_scores)))   # z-score per gene set (row)

annots <- combined_clustering$cluster_labels$KNN_cluster
names(annots) <- rownames(combined_clustering$cluster_labels)
ann_colors <- list(
  HLA_status = c(
    HLA_low        = "#0384fc",
...
ph <- pheatmap(ssgsea_z,
         clustering_distance_rows = "euclidean",
         clustering_distance_cols = "euclidean",
         clustering_method = "complete",color = viridis::inferno(100),
         scale = "none",cutree_cols = 4,
         border_color = NA,
...
col_clust <- cutree(ph$tree_col, k = 4)

## Find which column-cluster is “Undifferentiated”-like
undiff_row <- "Undifferentiated"
m_undiff <- tapply(ssgsea_z[undiff_row, ], col_clust, mean)
undiff_cluster_id <- as.integer(names(which.max(m_undiff)))

## Subset to HLA_low within that cluster
ids_undiff <- colnames(ssgsea_z)[
  col_clust == undiff_cluster_id
]
...
As HLA-II follows dedifferentiation, how well is this recapitulated by the signature in Kim et al, JCI 2021?
```{r Testing Kim et al signature, echo=F, fig.width=11}
###check if any of the melanoma dediff status enriches with the const+ genes (Tsoi et al, Cancer Cell 2018)
dediff_kim <- read.xlsx("../public/Kim_dediff_signature.xlsx")
dediff_kim <- list(dediff_kim$DediffGenes)
names(dediff_kim) <- "IFN_dediff"

gsparam_kim <- ssgseaParam(exprData = expr_mat, geneSets = dediff_kim)
...
rownames(annot_df_kim) <- colnames(ssgsea_z_kim)

ph <- pheatmap(ssgsea_z_kim,
         clustering_distance_cols = "euclidean",
         clustering_method = "complete",cluster_rows = F,color = viridis::inferno(100),
         scale = "none",cutree_cols = 4,
         border_color = NA,
         main = "Enrichment of Kim et al. JCI 2021", annotation_col = annot_df_kim,annotation_colors =ann_colors, 
         # annotation_col = sample_annot,   # uncomment if you created sample_annot
         fontsize_row = 10,
         fontsize_col = 8)

CIITA_counts <- assay(vst(combined_dds))["CIITA",]
IFN_dediff_score <- ssgsea_z_kim[ "IFN_dediff",]

IFN_dediff_score <- IFN_dediff_score[names(CIITA_counts)]

scatterdata <- rbind(CIITA_counts, IFN_dediff_score)
scatterdata <- as.data.frame(t(scatterdata))
scatterdata$Sample <- rownames(scatterdata)
combined_md <- combined_md[, !duplicated(names(combined_md))]
scatterdata<- scatterdata %>% dplyr::left_join(combined_md, by = "Sample")
...
Finally, we show that the HLA-II CONST+ phenotype overlaps with the Neural Crest-like phenotype
...
In undifferentiated TCLs, IFNg is not able to upregulate HLA-II (Figure S3)
```{r State-dependent coupling between IFN activity and HLA-II}
...
And we show potential mechanistic drivers relying in increased inflammatory potential (exploratory)

```{r potential inflammatory drivers p}
...
We also investigated the differential expression of HLA-II isotypes in our cohort and it showed a specific a higher expression of HLA-DR compared to the other isotypes

```{r HLA isotype comparison}
...
```{r analyses without undiff melanoma to exclude confounding due to diff status}
...
```{r DE HLA-II const+ and undiff HLA-low}
# undiff_tcls <- names(which(ssgsea_scores["Undifferentiated",] > ssgsea_group_z["Undifferentiated",]))
...
Finally, to confirm these results we studied the differences in expression of specific genes, linked with both dedifferentiation and inflammatory responses. Our results show that HLA-II positive melanomas have a constitutive activation of NFKB1, probably driving both dedifferentiation and CIITA activation

```{r violin plots on undiff - combined dds, echo=F}
create_violin_plot(combined_dds, "CXCL1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))
create_violin_plot(combined_dds, "NFKB1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))
create_violin_plot(combined_dds, "TRAF1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))

create_violin_plot(undiff_dds, "CXCL1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))+
  labs(title = "HLACONST+ vs Melanocytic melanoma")
create_violin_plot(undiff_dds_undiff, "CXCL1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))+
  labs(title = "HLACONST+ vs Undifferentiated melanoma")

create_violin_plot(undiff_dds, "NFKB1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))+
  labs(title = "HLACONST+ vs Melanocytic melanoma")
create_violin_plot(undiff_dds_undiff, "NFKB1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))+
  labs(title = "HLACONST+ vs Undifferentiated melanoma")
...
create_violin_plot(undiff_dds_undiff, "TRAF1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))
create_violin_plot(undiff_dds, "TRAF1", intgroup = "KNN_cluster")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = c("#0384fc", "red"))
